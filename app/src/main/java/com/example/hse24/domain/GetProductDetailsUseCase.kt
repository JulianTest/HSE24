...
class GetProductDetailsUseCase @Inject constructor(private val hseFromNetwork: HseFromNetwork
    ...
    override fun buildUseCase(param: String): Single<ProductDetails> {
        return hseFromNetwork.productDetails(param).map {
            ProductDetails(it.sku,it.nameShort,createImageUrl(it.imageUris.first()),it.productPrice.price.toInt(), it.longDescription)
        }
    }
